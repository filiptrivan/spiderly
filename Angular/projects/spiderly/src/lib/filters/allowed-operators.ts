 ...
 *
 * Deliberately a LEAF module (imports nothing but the codes) so every consumer can derive from
 * it without a cycle: the store's runtime validation and offered lists (`filter-store.ts`), the
 * compile-time `FilterRule.matchMode` union (`entities/filter-rule.ts`), and the data view's
 * kind-level lists (via `operatorOptionsForKind`). These used to be hand-kept copies; edit the
 * table here and every surface follows.
 *
