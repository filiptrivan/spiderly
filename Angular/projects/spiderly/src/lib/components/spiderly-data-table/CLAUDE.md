...
- **`isPending` is the single predicate.** It feeds PrimeNG's `[loading]` *and* the container's `[attr.aria-busy]`, so the visual and assistive signals cannot drift. `aria-busy` is new ground, not a restoration: PrimeNG's mask carries none and its spinner is `aria-hidden`. There is deliberately no opt-out `@Input`.
...
- **The overlay's styling is the `table-pending-veil` mixin** (`styles/layout/_mixins.scss`), shared with `spiderly-data-view`. PrimeNG's own `mask.background` token is `rgba(0,0,0,0.4)` light / `0.6` dark — modal strength, which makes the stale rows unreadable and so defeats the reason for keeping them. The mixin's doc comment carries the rest, including why `color` must be set alongside the background.
