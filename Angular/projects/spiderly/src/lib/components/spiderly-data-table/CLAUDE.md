...
- **`spiderly-data-view` shares the helper and the mixin, and is NOT covered by this spec.** It had the identical wrapper and the identical bug, but every spec here mounts `SpiderlyDataTableComponent`, so its `(onPage)` binding and its `@include` could both be deleted with the suite still green. Writing its first spec file was weighed and deferred (2026-08-29) — it has no consumer in the workspace and the generator never emits it — so the risk is knowingly carried, not covered. Wire a spec the moment it gains a consumer.
