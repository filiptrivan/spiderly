...
- **The shift state lives on the mousedown, not on the checkbox event.** `p-checkbox` emits the DOM `change` event as `onChange.originalEvent`, and `change` carries no `shiftKey` — so `onSelectionCellMouseDown` captures it into `pendingShift`, which the same click's `selectRow` consumes-and-clears. It is **id-paired** so the lifecycle is gesture-scoped: an aborted press (shift+mousedown that never completes the click) can't leak into a later toggle, and a keyboard toggle (Space, no press at all) never ranges. Don't "simplify" back to reading the event or to a bare boolean.
- **Ranges resolve over `renderedRows()` — the display window — never over raw `items` or `rowData.index`.** Lazy tables render exactly `items`; client-side tables render PrimeNG's `filteredValue` sliced by the paginator, so an `items`-based range would sweep filtered-out or off-page rows sitting between two visually adjacent ones (`rowData.index` can't address a range at all — only `loadFormArrayItems` stamps it). The invariant is validate-at-use: an anchor outside the window misses `findIndex` and the shift-click degrades to a plain toggle, so page flips and client-side sort/filter need no per-path resets. The only explicit resets are where rows are wholesale replaced — `lazyLoad` (the decided reset-on-reload semantics for lazy tables) and `loadFormArrayItems`.
...
- The selection `<td>` stops click propagation (a selection gesture must never double as `navigateOnRowClick` navigation) and suppresses the browser text selection at shift-mousedown, with `user-select: none` as belt. Known pre-existing sibling gap, deliberately not fixed here: the actions column has no such stop, so on a `navigateOnRowClick` table an action click also navigates — generalizing the arbitration into `onRowClick` is the right fix when that gets addressed.
