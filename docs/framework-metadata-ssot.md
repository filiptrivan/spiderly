...
## Type-zoo e2e fixture (same pipeline, different artifact)

`tests/e2e-fixtures/backend/entities/ZooShapes.cs` is a committed artifact generated by
`Spiderly.ZooGenerator` from the generators' own shape-axis data
(`SpiderlyTypeRef.ScalarKindByName` + a `[SpiderlyEnum]` enum): one entity property per supported
shape, non-nullable and nullable. The e2e job compiles it with the real toolchain (C#, EF
migration, generated Angular TS), so "no fixture ever had shape X" — how the nullable-enum
`prop?: MyEnum?;` TS17019 escape shipped — is structurally impossible for the shape axes. Adding
an axis member (a scalar name, a collection/wrapper name) regenerates the fixture automatically.
Guards: same regen script, CI diff, pre-commit auto-stage as the metadata artifacts, plus
`ZooFixtureTests` (unit-level drift check + the entities-TS emission-seam sweep).
