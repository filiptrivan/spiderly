...
## Considered and rejected: dropping external-auth handling (2026-06-07)

Briefly weighed pulling external-provider handling out of Spiderly entirely, motivated by "different consumers prefer different sign-in UX (popup vs full-page redirect)." **Rejected.** Popup-vs-redirect is an *acquisition-layer* concern — *how* the browser obtains the token/code — and it does **not** touch the security core (token signature/iss/aud/exp/nonce validation, JWKS rotation, `(provider, subject)` linking, the `email_verified` takeover gate, provisioning, session issuance). That core is the expensive, dangerous-to-hand-roll part, and **no mature framework** (ASP.NET Identity, NextAuth, Spring Security, Supabase) offloads it onto the app developer — see the reference table below. So the core stays framework-owned.

Acquisition-UX flexibility is **deferred** until a real second consumer needs it: at that point it's a thin seam addition (a third way to deliver a token/code to the **same** endpoints), not an engine change. Direction: ship **one** default acquisition mechanism (the server-side redirect code flow — the only one that works generically for confidential clients), keep it overridable via the decision-12 control ladder, and do **not** bless N mechanisms inside the framework. The earlier reason the question even arose ("we install Google/Facebook packages") was already false: there are no per-provider packages — Google/Facebook are pure config over `GenericOidcExternalAuthProvider`.
...
## Operational lessons (2026-06-10 — stale-binary incident + security re-grill)

Two durable conclusions, recorded so they aren't re-derived.

### 1. The registry validated at FIRST RESOLUTION, not at boot — fail-at-boot fix implemented (2026-06-11)

**Incident.** A backend running a pre-2026-06-07 build (before the `["facebook"]` preset existed) picked up a newly-added `{ "Code": "facebook" }` config entry via `reloadOnChange`, but its *compiled* `ExternalProviderPresets` had no facebook authority → `BuildGenericProvider` threw *"'facebook' has no 'Authority' and no known preset"*. It surfaced at **03:30 inside an unrelated Hangfire job** — not at boot, not at a login — because `ExternalAuthProviderRegistry` is a **lazy DI singleton**: its eager-in-the-ctor validation runs at *first resolution*, and the first thing to resolve it that night was a background job whose ctor chain pulls in a security service. `[AutomaticRetry]` then retried a deterministic ctor failure 3×. (New config + old binary is only fixed by a restart/rebuild — boot validation would NOT have prevented *this* incident; it fixes the failure mode the incident *revealed*.)

**Two comments used to lie.** The registry's XML summary said it "fails at startup rather than on the first login," and decision 3 above says misconfig is "mitigated by `ValidateOnStart` rejecting unknown/duplicate codes at boot." Both were **aspirational**: the `AddOptions<ExternalProviderOptions>().ValidateOnStart()` in `StartupExtensions` had **no `.Validate(...)` predicate**, so it guarded binding *shape* only; the real provider checks lived in the registry ctor and fired lazily. (Decision 3's line above is now superseded by this section; the registry summary + that `AddOptions` line carry comments describing the implemented design.)

**Fix (implemented 2026-06-11).** All provider invariants (authority/clientId presence, duplicate codes, "no authority AND no custom provider", custom-provider code present + unique) now live in `ExternalProviderOptionsValidator : IValidateOptions<ExternalProviderOptions>` (`Spiderly.Shared/ExternalAuth/`), **resolved from DI** so it also injects the consumer-registered `IEnumerable<IExternalAuthProvider>` and owns the cross-world rules. It's wired in `StartupExtensions` (`AddSingleton<IValidateOptions<…>, ExternalProviderOptionsValidator>()` next to the existing `ValidateOnStart()`), and `ExternalAuthProviderRegistry`'s ctor + helpers **no longer throw** — it's a pure builder that trusts the validated options (and `options.Value` re-runs the validator at materialization, so it can never see invalid config). Unit-tested in `Spiderly.Shared.Tests/ExternalProviderOptionsValidatorTests.cs` (each invariant + aggregation + the custom-provider escape hatch). Why this shape: validity no longer depends on *when* DI constructs the singleton, the ctor stays cheap (per MS DI guidelines), and a bad config present at boot fails **once with every problem aggregated**. Rejected: (a) a hosted-service that resolves the registry to force construction — works but buries validation in a ctor and reports one error at a time; (b) validating inside `AddSpiderly` at composition time — can't see custom providers, which register later.

**Honest scope — what this does and doesn't catch.** A misconfig *present at boot* (typo'd provider, or a stale binary deployed while the bad config is already in `appsettings`) now fails the **boot**, aggregated — the primary win. The exact incident above (binary booted *before* the config was edited, config then added at runtime via `reloadOnChange`) still surfaces at **first resolution**, because the registry injects `IOptions<T>` whose value materializes lazily and independently of the boot-time `ValidateOnStart` pass (separate cache from `IOptionsMonitor`). The improvement there is only that the lazy failure is now the same friendly aggregated `OptionsValidationException`, not a raw `InvalidOperationException` buried in a `[AutomaticRetry]` loop. Full runtime-edit immunity (freeze the provider set at boot) would require eagerly materializing `IOptions<ExternalProviderOptions>.Value` / the registry at startup — **deferred**, because production config is deploy-time, not hot-edited.

### 2. `TrustEmailVerified: true` for Facebook is sound — "sign in as someone else's email" can't start

Re-grilled the takeover surface; recording the conclusion so it isn't re-litigated. Facebook **requires email confirmation** before it will return an address in a token, and the `email` it returns is a confirmed one — it merely omits the `email_verified` *claim* (an OIDC implementation gap, not "we didn't verify"). So an attacker **cannot** get `victim@example.com` into a Facebook-signed token without controlling that mailbox. With the token signature/`iss`/`aud`/`exp` + the single-use server-issued nonce all validated (`VerifyExternalLoginNonce`), the spoofing, replay, login-CSRF, and audience-confusion chains are **all blocked by existing controls**.

The only residual is **email recycling** (an address reassigned to a new owner over time — e.g. `john@company.com` after John leaves the company): it is rare, affects **Google identically**, and is **not** closed by `email_verified` (both providers still report a years-ago-confirmed email as verified). So it's generic SSO account-linking hygiene, not a Facebook-specific or `TrustEmailVerified`-specific risk. `TrustEmailVerified` for Facebook recovers a true fact and is the same call NextAuth/Spring make.

**Therefore** the notify-on-link email + linked-providers/unlink UI are **standard account-settings polish** (do them when building out the account area), **not** a blocker for shipping Facebook login.
