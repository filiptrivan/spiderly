...
namespace Soft.SourceGenerators.Net
{
    ...
    public class TranslationsGenerator : IIncrementalGenerator
    {
        ...
        private static void Execute(IList<ClassDeclarationSyntax> classes, List<SoftClass> referencedProjectEntityClasses, string callingProjectDirectory, SourceProductionContext context)
        {
            if (classes.Count < 1)
                return;

            bool shouldGenerate = Helper.ShouldStartGenerator(nameof(TranslationsGenerator), classes);
            ...
            Helper.WriteResourceFile(dataEn, Path.Combine(sharedBusinessProjectPath, GetTermsFilePath(LanguageCodes.En)));
            Helper.WriteResourceFile(dataSrLatnRS, Path.Combine(sharedBusinessProjectPath, GetTermsFilePath(LanguageCodes.SrLatnRS)));
            ...
            Helper.WriteToTheFile(GetJsonFromDictionaryList(dataEn), Path.Combine(angulari18nFolderPath, GetAngulari18nFilePath(LanguageCodes.En)));
            Helper.WriteToTheFile(GetJsonFromDictionaryList(dataSrLatnRS), Path.Combine(angulari18nFolderPath, GetAngulari18nFilePath(LanguageCodes.SrLatnRS)));
        }

        private static Dictionary<string, string> GetTranslationData(SoftClass entity, LanguageCodes language)
        {
