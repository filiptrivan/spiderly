...
namespace Soft.SourceGenerators.Net
{
    ...
    public class TranslationsGenerator : IIncrementalGenerator
    {
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            IncrementalValueProvider<string> callingProjectDirectory = GetCallingPath(context);

            // Combine project directory with collected classes
            var combined = classDeclarations.Collect().Combine(callingProjectDirectory);

            context.RegisterImplementationSourceOutput(combined, static (spc, source) => Execute(source.Left, source.Right, spc));
        }

        private static void Execute(IList<ClassDeclarationSyntax> classes, string callingProjectDirectory, SourceProductionContext context)
        {
            if (classes.Count < 1)
                return;
            
            List<SoftClass> entities = Helper.GetSoftEntityClasses(classes);
...
""";
            ...
            var r = entities
                .Select(entity => new Dictionary<string, string>
                {
                    { $"{entity.Name}ExcelExportName", $"{entity.Name}" },
                    { $"{entity.Name}", "" },
                    { $"{entity}", "" }
                })
                .ToList();
            //context.AddSource($"GeneratedTerms.resx", SourceText.From(result, Encoding.UTF8));
            //context.AddSource($"Terms.sr-Latn-RS.generated.resx", SourceText.From(result, Encoding.UTF8));
        }

        private static string GetEnLanguageXmlData()
        {
            return $$"""
...
""";
        ...
        private static IncrementalValueProvider<string> GetCallingPath(IncrementalGeneratorInitializationContext context)
        {
