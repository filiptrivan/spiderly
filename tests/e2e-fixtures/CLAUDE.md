...
**Coverage is also judged per OPERATION on a mutable collection: add / no-op (omitted list) / removal** — an explicit empty list IS the removal case; "clear" is not a fourth cell to fill in. An add-leg assert says nothing about removal: the 2026-08-06 multiselect deselect regression (`c4e126bc`) shipped while every add-path check was green, because remove was uncovered at every layer at once (this suite's ordered-path spec asserted add-only; the Angular library had no control spec at all). The layers pin different halves and fail independently — that regression was purely client-side, invisible to any API-level spec here. API legs pin the backend's removal/null semantics; the library's Karma specs pin the control-to-form commit; the seam they meet at is the request body, which is why a UI-driven leg was deliberately not added.
