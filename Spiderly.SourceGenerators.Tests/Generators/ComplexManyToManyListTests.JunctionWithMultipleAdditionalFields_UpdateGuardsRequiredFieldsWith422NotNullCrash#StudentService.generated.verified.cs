...
namespace TestApp.Business.Services
{
    ...
    public class StudentServiceGenerated : ServiceBase
    {
        ...
        /// <see cref="OnAfterStudentListDelete"/> with a one-element list, so override
        /// only the list hook unless single-id and batch flows genuinely diverge.
        /// </summary>
        /// <param name="id">The ID of the entity that was deleted</param>
        public virtual Task OnAfterStudentDelete(long id) =>
            OnAfterStudentListDelete(id.StructToList());
        ...
        /// <example>
        /// public override async Task OnAfterStudentListDelete(List&lt;long&gt; deletedIds)
        /// {
        ...
        public async virtual Task DeleteStudentList(List<long> listForDelete_1)
        {
            await _deps.Context.WithTransactionAsync(async () =>
            {
                ...
                await OnAfterStudentListDelete(listForDelete_1);
