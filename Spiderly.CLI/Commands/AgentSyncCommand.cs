    ...
    internal static class AgentSyncCommand
    {
        ...
        public static int Execute(string projectRoot = null, string agentRoot = null, bool saveAgentRoot = false, bool failIfMissing = true)
        {
            ...
            try
            {
                ...
                // The spiderly-* junctions point at per-machine absolute node_modules paths, so they must
                // never be committed. A freshly-init'd app's .gitignore already excludes them, but an outer
                // --agent-root target (umbrella/monorepo) has no such guarantee — ensure it in the target.
                EnsureGitignorePattern(target, "**/.claude/skills/spiderly-*",
                    "Spiderly machine-local skill junctions (regenerated by 'spiderly agent-sync')");
            }
            catch (Exception ex)
            {
        ...
        private static void EnsureLocalConfigIgnored(string source) =>
            EnsureGitignorePattern(source, ".spiderly/*.local.json", "Spiderly machine-local config (agent-sync workspace target, etc.)");

        /// <summary>Ensures <paramref name="root"/>'s .gitignore lists <paramref name="pattern"/> (idempotent, with a header comment).</summary>
        private static void EnsureGitignorePattern(string root, string pattern, string comment)
        {
