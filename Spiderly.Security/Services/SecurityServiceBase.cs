...
namespace Spiderly.Security.Services
{
    ...
    public class SecurityServiceBase<TUser> where TUser : class, IUser, new()
    {
        ...
        #region Google OAuth Authorization Code Flow

        public string BuildGoogleOAuthUrl(string redirectUri, string returnUrl, string browserId)
        {
            string clientId = SettingsProvider.Current.GoogleClientId;

            GoogleOAuthStateDTO stateData = new GoogleOAuthStateDTO
            {
                ReturnUrl = returnUrl,
                BrowserId = browserId,
                Nonce = Guid.NewGuid().ToString()
            };
            string state = EncodeOAuthState(stateData);
        ...
        public async Task<GoogleTokenResponseDTO> ExchangeGoogleCodeForTokensAsync(string code, string redirectUri)
        {
            string clientId = SettingsProvider.Current.GoogleClientId;
            string clientSecret = SettingsProvider.Current.GoogleClientSecret;
            ...
            {
                throw new BusinessException($"Google token exchange failed: {responseContent}");
            }

            return JsonSerializer.Deserialize<GoogleTokenResponseDTO>(responseContent)!;
        }

        public async Task<AuthResultDTO> LoginWithGoogleCode(string code, string redirectUri, string browserId)
        {
            GoogleTokenResponseDTO tokenResponse = await ExchangeGoogleCodeForTokensAsync(code, redirectUri);

            ExternalProviderDTO externalProviderDTO = new ExternalProviderDTO
            {
                IdToken = tokenResponse.id_token,
                BrowserId = browserId
            };

            return await LoginExternal(externalProviderDTO);
        }

        public string EncodeOAuthState(GoogleOAuthStateDTO stateData)
        {
            string json = JsonSerializer.Serialize(stateData);
            return Convert.ToBase64String(Encoding.UTF8.GetBytes(json));
        }

        public GoogleOAuthStateDTO DecodeOAuthState(string state)
        {
            string json = Encoding.UTF8.GetString(Convert.FromBase64String(state));
            return JsonSerializer.Deserialize<GoogleOAuthStateDTO>(json)!;
        }
