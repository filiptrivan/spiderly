...
**Unknown fields are rejected, not ignored.** A filter key or `MultiSortMeta.Field` that matches no generated case, and any invalid `MatchMode`, throws `BusinessException` (HTTP 400) with a message naming the offender and listing the valid fields — never a silent no-op (a silently ignored filter would return unfiltered rows the caller treats as filtered). The throws come from `PaginationErrors` (`Spiderly.Shared.Exceptions`) and carry `ApiErrorCodes.UnknownFilterField` / `UnknownSortField` / `InvalidMatchMode` in `ApiErrorDTO.errorCode`, so clients branch on the code rather than the message text. `*CommaSeparated` collection fields are filterable but **not sortable**. If a hand-written override consumes a pseudo filter key the generated switch doesn't know, it must `filterDTO.Filters.Remove("...")` that key before calling `base.GetPaginated{Entity}List(...)`, or the base will reject it.
